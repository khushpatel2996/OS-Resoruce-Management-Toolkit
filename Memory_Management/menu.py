...
def memory_allocation():
    ...
    exe_path = BASE_DIR / "memory_allocation.exe"
    subprocess.run(
        ["gcc", "memory_allocation.c", "-o", exe_path.name],
        cwd=BASE_DIR,
        check=True,
    )
    subprocess.run([str(exe_path)], cwd=BASE_DIR, check=True)
...
def page_replacement():
    ...
    # compile and run C program
    exe_path = BASE_DIR / "page_replacement.exe"
    subprocess.run(
        ["gcc", "page_replacement.c", "-o", exe_path.name],
        cwd=BASE_DIR,
        check=True,
    )
    subprocess.run([str(exe_path)], cwd=BASE_DIR, check=True)
